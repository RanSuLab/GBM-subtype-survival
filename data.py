...
data_path = 'D:/aa_papar/segmentation/data/HGG'
save_path = 'D:/aa_papar/segmentation/data/HGG/unet/'
image_rows = 240
image_cols = 240
...
def create_train_data():
    train_data_path = os.path.join(data_path, 'train_new')
    ...
    for image_name in images:
        ...
        if 'seg' in image_name:
            continue
        print(image_name.split('.')[0][:20])
        image_mask_name = image_name.split('.')[0][:20] + '_seg_82.jpg'
...
def create_test_data():
    ...
    print('-'*30)
    for image_name in images:
        print(image_name.split('.')[0][15:18])
        img_id = int(image_name.split('.')[0][15:18])
        img = cv2.imread(os.path.join(train_data_path, image_name), cv2.IMREAD_GRAYSCALE)
