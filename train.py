...
def augmentation(image, imageB, org_width=240,org_height=240, width=190, height=262):
    max_angle=20
    ...
    print('111111111111',image.shape)
    ...
    print('222222222222', image.shape)
    ...
    print('444444444444', image.shape)
...
def train_and_predict():
    ...
    imgs_mask_train=np.load("./mask.npy")
    ...
    mgs_train = preprocess(imgs_train)
    imgs_mask_train = preprocess(imgs_mask_train)
    print(np.histogram(imgs_train))
    print(np.histogram(imgs_mask_train))

    total=mgs_train.shape[0]
    mgs_train/=255.
    mean = mgs_train.mean()# (0)[np.newaxis,:]  # mean for data centering
    std = np.std(mgs_train)  # std for data normalization
    mgs_train -= mean
    mgs_train /= std
    ...
    for i in range(max_iters):
        ...
        for img in range(batch_size):
            ...
            data_batch[img,0],mask_batch[img,0]=augmentation(mgs_train[idx,0],imgs_mask_train[idx,0])
            # plt.subplot(121)
    ...
    imgs_test -= np.load('./mean.npy')
    imgs_test /=np.load('./std.npy')
